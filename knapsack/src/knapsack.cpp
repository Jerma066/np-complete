...
int Knapsack::Bound(BABNode node) {
    ...
    while ((j < elements_.size()) && (total_weight + elements_[j].weight < capacity_)) {
    ...
    if (j < elements_.size()) {
        profit_bound += (capacity_ - total_weight) * elements_[j].profit/elements_[j].weight;
    }
...
size_t Knapsack::BranchAndBound() {
	std::sort(elements_.begin(), elements_.end(), cmp);
    std::queue<BABNode> Q;
    BABNode u, v;
    int maxProfit = 0;
    int maxWeight = 0;
